...
sequenceDiagram
  ...
  O->>V: ov add-resource + выводы в память
```
...
- Роль: память (предпочтения, обратная связь, проектные ограничения, ссылки) и индекс спек, архитектуры, `.agents`. Спеки в нём не живут.
...
- Синхронизация: `ov add-resource ~/specs/<p> --to viking://resources/<p>/specs` с `parse_mode=no_split` после этапов.
...
- Известные риски: Alpha и частые ломающие изменения; каждая запись вызывает LLM; issue #4482 (поиск по файлам); на локальных LLM извлечение памяти не работает (#4879), поэтому LLM облачная.
...
- **Смоук и OpenViking.** `~/specs/opencode-smoke` — симлинк на `/tmp`, контейнер монтирует `~/specs` и за симлинк не выходит, поэтому `ov-sync` в смоуке падает с `Local path does not exist`. Для проектов, лежащих прямо в `~/specs`, это не так; смоук память не засоряет.
