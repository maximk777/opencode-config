...
- **Отчёты задач append-only.** Исполнитель дописывает `## Attempt <n>`, ревьюер — `## Review round <n>` с вердиктом. `change-state` решает по последнему вердикту: `awaiting_review` (вердикта нет или после него новая попытка), `needs_fix`, `ready_to_accept`.
- **Смоук доказывает отказ ревьюера.** Задача 1.5 приходит уже сделанной без сценария `inverted range` (`smoke/pending`), отчёт без вердикта. Проверка требует `Verdict: NOT COMPLIANT` хотя бы в одном отчёте и два коммита в репе спек.
