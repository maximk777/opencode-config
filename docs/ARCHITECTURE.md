...
1. Минимум своего кода. Инструкции, конфиг и короткие скрипты. Прошлый HarnessKit (внутренний репозиторий, 18 пакетов Go) показал, куда ведёт обратное.
...
- в рабочей репе никто не коммитит без просьбы; оркестратор фиксирует принятые задачи через `git add -- <файлы>`, а после принятия предложенных коммитов коммитит и пушит сам, без запроса прав;
...
- сводка заканчивается секцией Next с точным следующим шагом потока (директива из `buildCompactionContext`);
- на `session.compacted` помечает сессию: если в течение 180 секунд приходит `session.idle` (ход после компакции закончился текстом без работы) и с прошлого авто-продолжения прошло больше 120 секунд, плагин сам отправляет в сессию приказ продолжения через `prompt_async` («Compaction finished; it is not a gate…»), и поток идёт дальше без реплики пользователя; модель, продолжившая работу сама, авто-продолжения не получает — метка протухает;
...
Проверено в исходниках OpenCode 1.18.30: хук `experimental.session.compacting` принимает `output.context[]` и `output.prompt`, событие `session.compacted` публикуется в `session/compaction.ts:554`, эндпоинт `summarize` есть в SDK. Продолжение после компакции — исходники 1.18.31: `summarize` сам запускает `promptSvc.loop`, а ядро добавляет синтетическое сообщение «Continue if you have next steps, or stop and ask for clarification…», по которому модель часто останавливается.
...
- **Как OpenCode проверяет bash (исходники 1.18.30).** Команда разбирается tree-sitter на отдельные команды, каждая сверяется со своими правилами вместе с редиректами; решает последнее совпавшее правило, `*` проходит через пробелы и `/`. Отсюда правила:
  ...
  - в конце всех агентов, кроме оркестратора, `deny` на `env`, `printenv`, `*.env`, `git commit*`, `git * commit*`, `git * push*`; у оркестратора гит открыт целиком (`*` allow, диалогов нет), коммиты спек — по-прежнему только через `specs-commit`; у исполнителей bash открыт, запреты — страховка, итог проверяет неподвижный HEAD;
...
- **Компакция и продолжение (исходники 1.18.31).** После компакции ядро само добавляет синтетическое сообщение «Continue if you have next steps, or stop and ask for clarification if you are unsure» — модель часто выбирает «stop», и сессия замолкает до реплики пользователя. Лечится в плагине: приказ продолжения на `session.idle` в окне 180 секунд после `session.compacted`, с кулдауном 120 секунд.
...
- **OpenViking.**
  ...
  - Плагин сброса писал сводки в `viking://user/memories/sessions` с `mode: upsert`, сервер отвечал `INVALID_URI` и `unsupported write mode`, ошибка глоталась. Теперь URI `viking://user/<user>/memories/sessions/<id>.md` (пользователь из `/api/v1/system/status`), режим `replace`, ключ из `~/.openviking/mcp-key`, сбой пишется в лог. Не-scoped сессии (build, executor) на `session.idle` оставляют артефакт: заголовок сессии и последние ходы (запрос ≤280 симв., ответ ≤1200, ≤16 сообщений, ≤10 КБ) тем же `content/write` в `replace`; запись не чаще раза в 15 минут на сессию и только при изменении контента (fingerprint в памяти процесса). Оба пути пишут с `processing_mode: vectors_only` (#3615): без VLM-суммаризации и `remember`, только локальные эмбеддинги Ollama, внешних вызовов LLM нет.
