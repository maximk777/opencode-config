...
Дата дизайна: 2026-09-13. Статус: дизайн согласован, следующий шаг — план реализации.
...
```jsonc
{
  "agent": {
    "executor": {
      ...
      "steps": 40,
      "permission": { "task": "deny", "bash": { "*": "ask", "git commit*": "deny" } }
    }
  }
}
```
...
| 1 | репа `~/setup-opencode`, запуск через `OPENCODE_CONFIG_DIR` | `opencode debug agent orchestrator` верен |
