...
SCREEN_PATH = "domains/operations/map/documents.md"
...
TRANSITIONS = "## Transitions\n\n| Action | Target |\n|---|---|\n| Open | screen:operations/documents |\n"
...
class CheckMapTest(unittest.TestCase):
    ...
    def stream(self, stage="goal"):
        self.write_json("domains/operations/streams/docs/stream.json", {
            "key": "stream:operations/docs",
            "profile": "ui-migration",
            "stage": stage,
            "scope": ["screen:operations/documents"],
            "approvals": [],
        })
        self.write("domains/operations/streams/docs/stories/documents/story.md", STORY)
    ...
    def test_empty_story_field_after_decomposition(self):
        for stage in ("ready", "delivery", "done"):
            with self.subTest(stage=stage):
                ...
                self.write(SCREEN_PATH, screen(override={"story": ""}))
                self.assertEqual(self.findings(), [
                    SCREEN_PATH + ":10 map field story is empty after decomposition of stream:operations/docs"])

    def test_empty_story_field_outside_closed_stream(self):
        self.stream(stage="ready")
        self.write("domains/operations/map/other.md", screen(override={"key": "screen:operations/other", "story": ""}))
        self.write(SCREEN_PATH, screen())
        self.assertEqual(self.findings(), [])
    ...
    def test_story_naming_missing_story(self):
        ...
        self.assertTrue(any("story:operations/documents" in line for line in lines), lines)
    ...
    def test_pipe_in_list_item_breaks_map_row(self):
        self.stream()
        self.write(SCREEN_PATH, screen(override={"parent": "[screen:operations/a | b]"}))
    ...
    def test_key_path_mismatch(self):
        self.stream()
        self.write(SCREEN_PATH, screen(override={"key": "screen:operations/other"}))
        self.assert_finding_at(SCREEN_PATH + ":2 map ")

    def test_checklist_element_uses_its_own_fields(self):
        self.write("domains/ops/rules/audit.md", "---\nkey: rule:ops/audit\nowner:\nstory:\n---\n\n## Checks\n\n"
                   "| Check | Target |\n|---|---|\n")
        self.write(".agents/profiles/checklist/profile.json", FIXTURE.read_text(encoding="utf-8"))
        self.assert_finding_at("domains/ops/rules/audit.md:3 map ")
        self.write("domains/ops/rules/audit.md", "---\nkey: rule:ops/audit\nowner: team\nstory:\n---\n\n## Checks\n\n"
                   "| Check | Target |\n|---|---|\n")
        self.assertEqual(self.findings(), [])

    def test_map_without_markers(self):
        self.write("domains/risks/MAP.md", "# Map of domain risks\n\n## Legacy trace\n")
    ...
    def test_indented_marker_gives_finding(self):
        self.write("domains/risks/MAP.md", "# Map\n\n  <!-- map:screens:begin -->\n<!-- map:screens:end -->\n")
        self.assert_finding_at("domains/risks/MAP.md:1 map ")
    ...
    def test_generate_renders_map_that_check_accepts(self):
        ...
        text = (self.ws / "domains/operations/MAP.md").read_text(encoding="utf-8")
        self.assertIn("| screen:operations/documents | /documents | place |", text)
        self.assertIn("| screen:operations/documents | Open | screen:operations/documents |", text)
