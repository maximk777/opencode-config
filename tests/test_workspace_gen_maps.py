import sys
from pathlib import Path

REPO = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(REPO / "kits/workspace/tools"))
sys.path.insert(0, str(Path(__file__).resolve().parent))

import json
import subprocess
import tempfile
import unittest

import generate
from wslib import gen_maps, rules_generated
from wslib.common import Context

SCRIPT = REPO / "kits/workspace/tools/generate.py"
FIXTURE = Path(__file__).resolve().parent / "fixtures/profiles/checklist/profile.json"

MAP_REL = "projects/abs/domains/operations/MAP.md"
STREAM_REL = "projects/abs/domains/operations/streams"

UI_MIGRATION = {
    "name": "ui-migration",
    "elements": [
        {
            "kind": "screen",
            "prefix": "screen",
            "dir": "map",
            "fields": ["key", "route", "kind", "section", "parent", "access", "label", "wave", "story"],
            "may_be_empty": ["parent", "story"],
            "values": {"kind": ["place"]},
            "tables": [{"heading": "Transitions", "columns": ["Action", "Target"], "keys": ["Target"]}],
        }
    ],
    "story": {
        "fields": ["key", "type", "wave", "tracker", "scope", "depends", "repos", "decisions", "mockups"],
        "may_be_empty": ["tracker", "scope", "depends", "repos", "decisions", "mockups"],
        "values": {"type": ["story"]},
        "sections": ["Goal", "Scope", "Acceptance criteria", "Verification", "Out of scope", "Open questions"],
    },
    "epic": {
        "sections": ["Goal", "Scope", "Success criteria", "Out of scope", "Open questions", "Target users", "Product"]
    },
    "map_doc": {
        "tables": [
            {
                "heading": "Legacy trace",
                "columns": ["Legacy group", "Legacy item", "Legacy route", "Target"],
                "target": "Target",
            }
        ]
    },
    "stages": {
        "goal": [{"gate": "epic_sections"}, {"gate": "approval"}],
        "map": [
            {"gate": "unique_keys"},
            {"gate": "no_dangling_targets"},
            {"gate": "legacy_traced", "table": "Legacy trace"},
            {"gate": "approval"},
        ],
        "decomposition": [{"gate": "two_way_coverage"}, {"gate": "approval"}],
        "ready": [{"gate": "tracker_ids"}, {"gate": "no_open_questions", "section": "Open questions"}],
        "delivery": [],
        "done": [{"gate": "work_records"}],
    },
}

SCREEN_LIST = """---
key: screen:abs/operations/list
route: /operations
kind: place
section: Operations
parent:
access: listOperations
label: Operations
wave: 1
story:
---
# Operations list

## Transitions

| Action | Target |
|---|---|
| Open | screen:abs/operations/card |
| Back | screen:abs/clients/home |
"""

SCREEN_CARD = """---
key: screen:abs/operations/card
route: /operations/:id
kind: place
section: Operations
parent: screen:abs/operations/list
access: getOperation
label: Operation
wave: 1
story: story:abs/operations/documents
---
# Operation card

## Notes

No table here.
"""

SCREEN_EXPORT = """---
key: screen:abs/operations/export
route: /operations/export
kind: place
section: Operations
parent: screen:abs/operations/list
access: exportOperations
label: Export
wave: 2
story:
---
# Export

## Transitions

| Action | Target |
|---|---|
| Done | screen:abs/operations/list |
"""

MAP_HEAD = "# Map of domain operations\n\nHand-written intro.\n\n"
MAP_TAIL = (
    "\n## Legacy trace\n"
    "| Legacy group | Legacy item | Legacy route | Target |\n"
    "|---|---|---|---|\n"
    "| Ops | List | /old/ops | screen:abs/operations/list |\n"
)
MAP_SOURCE = (
    MAP_HEAD
    + "## Screens\n<!-- map:screens:begin -->\nstale\n<!-- map:screens:end -->\n\n"
    + "## Transitions\n<!-- map:transitions:begin -->\n<!-- map:transitions:end -->\n"
    + MAP_TAIL
)

SCREENS_TABLE = (
    "| key | route | kind | section | parent | access | label | wave | story |\n"
    "|---|---|---|---|---|---|---|---|---|\n"
    "| screen:abs/operations/card | /operations/:id | place | Operations | screen:abs/operations/list | getOperation"
    " | Operation | 1 | story:abs/operations/documents |\n"
    "| screen:abs/operations/list | /operations | place | Operations |  | listOperations | Operations | 1 |  |\n"
)
TRANSITIONS_TABLE = (
    "| From | Action | Target |\n"
    "|---|---|---|\n"
    "| screen:abs/operations/list | Open | screen:abs/operations/card |\n"
    "| screen:abs/operations/list | Back | screen:abs/clients/home |\n"
)

STORY_DOCUMENTS = """---
key: story:abs/operations/documents
type: story
wave: 1
tracker:
scope: [screen:abs/operations/card, screen:abs/operations/list]
unmapped: Export to Excel
depends: []
repos: []
decisions: []
mockups: []
---
# Documents

## Goal

Documents.
"""

STORY_REPORTS = """---
key: story:abs/operations/reports
type: story
wave: 2
tracker: TASK-2
scope:
  - screen:abs/operations/list
depends: [story:abs/operations/documents]
repos: []
decisions: []
mockups: []
---
# Reports

## Goal

Reports.
"""

BREAKDOWN_HEAD = (
    "# Breakdown of stream:abs/operations/main\n"
    "\n"
    "Generated by tools/generate.py from stories/*/story.md. Do not edit.\n"
    "\n"
    "| Story | Wave | Scope | Unmapped | Tracker | Depends |\n"
    "|---|---|---|---|---|---|\n"
)
BREAKDOWN_ROWS = (
    "| story:abs/operations/documents | 1 | screen:abs/operations/card, screen:abs/operations/list"
    " | Export to Excel |  |  |\n"
    "| story:abs/operations/reports | 2 | screen:abs/operations/list |  | TASK-2 | story:abs/operations/documents |\n"
)
RU_BREAKDOWN = {
    "title": "Разбивка",
    "note": "Сгенерировано tools/generate.py из stories/*/story.md. Не редактировать.",
    "columns": ["Стори", "Волна", "Экраны", "Без экрана", "Трекер", "Зависит от"],
}


def stream_json(project, domain, stream):
    data = {
        "key": "stream:%s/%s/%s" % (project, domain, stream),
        "profile": "ui-migration",
        "stage": "goal",
        "scope": [],
        "approvals": [],
    }
    return json.dumps(data, indent=2, ensure_ascii=False) + "\n"


def strip_between_markers(text):
    """Drop lines strictly between map begin and end markers."""
    kept = []
    inside = False
    for line in text.split("\n"):
        if line.startswith("<!-- map:") and line.endswith(":end -->"):
            inside = False
        if not inside:
            kept.append(line)
        if line.startswith("<!-- map:") and line.endswith(":begin -->"):
            inside = True
    return "\n".join(kept)


class MapsTestCase(unittest.TestCase):
    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self.root = Path(self._tmp.name).resolve()
        self.write(".agents/kit.json", json.dumps({"name": "workspace", "version": "0.1.0", "params": {}}) + "\n")
        self.write(
            ".agents/profiles/ui-migration/profile.json",
            json.dumps(UI_MIGRATION, indent=2, ensure_ascii=False) + "\n",
        )
        self.write("projects/abs/domains/operations/map/list.md", SCREEN_LIST)
        self.write("projects/abs/domains/operations/map/card.md", SCREEN_CARD)
        self.write(MAP_REL, MAP_SOURCE)
        self.write(STREAM_REL + "/main/stream.json", stream_json("abs", "operations", "main"))
        # Written in reverse order so the output order cannot come from creation order.
        self.write(STREAM_REL + "/main/stories/reports/story.md", STORY_REPORTS)
        self.write(STREAM_REL + "/main/stories/documents/story.md", STORY_DOCUMENTS)

    def tearDown(self):
        self._tmp.cleanup()

    def write(self, rel, text):
        path = self.root / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(text, encoding="utf-8")

    def write_out(self, out):
        for rel, data in out.items():
            path = self.root / rel
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(data)


class MapTablesTest(MapsTestCase):
    def test_exact_map_tables(self):
        out = gen_maps.render(self.root)
        expected = (
            MAP_HEAD
            + "## Screens\n<!-- map:screens:begin -->\n" + SCREENS_TABLE + "<!-- map:screens:end -->\n\n"
            + "## Transitions\n<!-- map:transitions:begin -->\n" + TRANSITIONS_TABLE + "<!-- map:transitions:end -->\n"
            + MAP_TAIL
        )
        self.assertEqual(out[MAP_REL], expected.encode("utf-8"))

    def test_rows_carry_project_screen_keys(self):
        # Elements under projects/abs/domains/operations/ must land in that project's MAP.md
        # with their screen:abs/... keys; empty tables here would mean a generator bug.
        out = gen_maps.render(self.root)[MAP_REL].decode("utf-8")
        self.assertIn("\n| screen:abs/operations/card | /operations/:id ", out)
        self.assertIn("\n| screen:abs/operations/list | /operations ", out)
        self.assertNotIn("screen:operations/", out)

    def test_added_screen_changes_only_lines_between_markers(self):
        self.write_out(gen_maps.render(self.root))
        before = (self.root / MAP_REL).read_text(encoding="utf-8")
        self.write("projects/abs/domains/operations/map/export.md", SCREEN_EXPORT)
        after = gen_maps.render(self.root)[MAP_REL].decode("utf-8")
        self.assertNotEqual(after, before)
        self.assertEqual(strip_between_markers(after), strip_between_markers(before))
        self.assertTrue(after.endswith(MAP_TAIL))
        self.assertIn("| screen:abs/operations/export | Done | screen:abs/operations/list |\n", after)
        self.assertLess(after.index("| screen:abs/operations/card | /"), after.index("| screen:abs/operations/export | /"))
        self.assertLess(after.index("| screen:abs/operations/export | /"), after.index("| screen:abs/operations/list | /"))

    def test_absent_marker_pair_is_skipped(self):
        text = (
            "# Map\n<!-- map:screens:begin -->\n<!-- map:screens:end -->\n"
            "<!-- map:transitions:begin -->\nhand text\n"
        )
        self.write(MAP_REL, text)
        out = gen_maps.render(self.root)
        expected = (
            "# Map\n<!-- map:screens:begin -->\n" + SCREENS_TABLE + "<!-- map:screens:end -->\n"
            "<!-- map:transitions:begin -->\nhand text\n"
        )
        self.assertEqual(out[MAP_REL], expected.encode("utf-8"))

    def test_map_without_expected_begin_markers_is_not_rendered(self):
        self.write(MAP_REL, "# Map\n<!-- map:rules:begin -->\n<!-- map:rules:end -->\n")
        self.assertNotIn(MAP_REL, gen_maps.render(self.root))

    def test_same_domain_in_two_projects_render_their_own_files(self):
        web_map = "projects/web/domains/operations/MAP.md"
        self.write("projects/web/domains/operations/map/list.md", SCREEN_LIST.replace("abs", "web"))
        self.write(web_map, "# Web map\n\n## Screens\n<!-- map:screens:begin -->\nstale\n<!-- map:screens:end -->\n")
        self.write(
            "projects/web/domains/operations/streams/web-main/stream.json",
            stream_json("web", "operations", "web-main"),
        )
        self.write(
            "projects/web/domains/operations/streams/web-main/stories/documents/story.md",
            STORY_DOCUMENTS.replace("abs", "web"),
        )
        out = gen_maps.render(self.root)
        self.assertEqual(sorted(rel for rel in out if rel.endswith("MAP.md")), sorted([MAP_REL, web_map]))
        abs_map = out[MAP_REL].decode("utf-8")
        web = out[web_map].decode("utf-8")
        self.assertIn("| screen:abs/operations/list |", abs_map)
        self.assertNotIn("screen:web/", abs_map)
        self.assertIn("| screen:web/operations/list |", web)
        self.assertNotIn("screen:abs/", web)
        self.assertIn(STREAM_REL + "/main/BREAKDOWN.md", out)
        self.assertIn("projects/web/domains/operations/streams/web-main/BREAKDOWN.md", out)

    def test_checklist_domain_renders_rules_and_checks(self):
        self.write(".agents/profiles/checklist/profile.json", FIXTURE.read_text(encoding="utf-8"))
        self.write(
            "projects/abs/domains/ops/rules/r1.md",
            "---\nkey: rule:abs/ops/r1\nowner: me\nstory:\n---\n# R1\n\n## Checks\n\n"
            "| Check | Target |\n|---|---|\n| Lint | screen:abs/operations/list |\n",
        )
        self.write(
            "projects/abs/domains/ops/MAP.md",
            "# Ops\n<!-- map:rules:begin -->\n<!-- map:rules:end -->\n<!-- map:checks:begin -->\n<!-- map:checks:end -->\n",
        )
        out = gen_maps.render(self.root)
        expected = (
            "# Ops\n<!-- map:rules:begin -->\n"
            "| key | owner | story |\n|---|---|---|\n| rule:abs/ops/r1 | me |  |\n"
            "<!-- map:rules:end -->\n<!-- map:checks:begin -->\n"
            "| From | Check | Target |\n|---|---|---|\n| rule:abs/ops/r1 | Lint | screen:abs/operations/list |\n"
            "<!-- map:checks:end -->\n"
        )
        self.assertEqual(out["projects/abs/domains/ops/MAP.md"], expected.encode("utf-8"))

    def with_from_column(self, value, language):
        profile = json.loads(json.dumps(UI_MIGRATION))
        profile["map_doc"]["from_column"] = value
        self.write(".agents/profiles/ui-migration/profile.json", json.dumps(profile, indent=2, ensure_ascii=False) + "\n")
        params = {} if language is None else {"language": language}
        self.write(".agents/kit.json", json.dumps({"name": "workspace", "version": "0.1.0", "params": params}) + "\n")
        return gen_maps.render(self.root)[MAP_REL].decode("utf-8")

    def test_russian_workspace_gets_localized_from_column(self):
        out = self.with_from_column({"en": "From", "ru": "Откуда"}, "ru")
        self.assertIn("<!-- map:transitions:begin -->\n| Откуда | Action | Target |\n|---|---|---|\n", out)
        self.assertNotIn("| From |", out)

    def test_english_workspace_keeps_from(self):
        for language in ("en", None):
            out = self.with_from_column({"en": "From", "ru": "Откуда"}, language)
            self.assertIn("<!-- map:transitions:begin -->\n" + TRANSITIONS_TABLE, out, language)

    def test_from_column_comes_from_the_profile_declaring_the_kind(self):
        checklist = json.loads(FIXTURE.read_text(encoding="utf-8"))
        checklist["map_doc"]["from_column"] = "Source"
        self.write(".agents/profiles/checklist/profile.json", json.dumps(checklist) + "\n")
        self.write(
            "projects/abs/domains/ops/rules/r1.md",
            "---\nkey: rule:abs/ops/r1\nowner: me\nstory:\n---\n# R1\n",
        )
        self.write("projects/abs/domains/ops/MAP.md", "# Ops\n<!-- map:checks:begin -->\n<!-- map:checks:end -->\n")
        out = gen_maps.render(self.root)
        self.assertIn("| Source | Check | Target |\n", out["projects/abs/domains/ops/MAP.md"].decode("utf-8"))
        self.assertIn(TRANSITIONS_TABLE, out[MAP_REL].decode("utf-8"))


class MapSourceShapesTest(MapsTestCase):
    def expected_map(self, screens=SCREENS_TABLE, transitions=TRANSITIONS_TABLE):
        return (
            MAP_HEAD
            + "## Screens\n<!-- map:screens:begin -->\n" + screens + "<!-- map:screens:end -->\n\n"
            + "## Transitions\n<!-- map:transitions:begin -->\n" + transitions + "<!-- map:transitions:end -->\n"
            + MAP_TAIL
        )

    def test_crlf_map_round_trips_with_only_block_lines_changed(self):
        (self.root / MAP_REL).write_bytes(MAP_SOURCE.replace("\n", "\r\n").encode("utf-8"))
        out = gen_maps.render(self.root)[MAP_REL]
        self.assertEqual(out, self.expected_map().replace("\n", "\r\n").encode("utf-8"))
        self.write_out({MAP_REL: out})
        self.assertEqual(gen_maps.render(self.root)[MAP_REL], out)
        stale = [f for f in rules_generated.check_generated(Context(self.root)) if f.path == MAP_REL]
        self.assertEqual(stale, [])

    def test_reordered_source_columns_render_in_profile_order(self):
        self.write(
            "projects/abs/domains/operations/map/list.md",
            SCREEN_LIST.replace("| Action | Target |", "| Target | Action |")
            .replace("| Open | screen:abs/operations/card |", "| screen:abs/operations/card | Open |")
            .replace("| Back | screen:abs/clients/home |", "| screen:abs/clients/home | Back |"),
        )
        out = gen_maps.render(self.root)[MAP_REL].decode("utf-8")
        self.assertEqual(out, self.expected_map())

    def test_transitions_table_with_parse_error_renders_well_formed_rows(self):
        self.write(
            "projects/abs/domains/operations/map/list.md",
            SCREEN_LIST.replace("| Back |", "| Broken | a | b |\n| Back |"),
        )
        out = gen_maps.render(self.root)[MAP_REL].decode("utf-8")
        self.assertEqual(out, self.expected_map())

    def test_element_with_broken_frontmatter_is_excluded(self):
        self.write(
            "projects/abs/domains/operations/map/card.md",
            SCREEN_CARD.replace("access: getOperation", "access getOperation"),
        )
        out = gen_maps.render(self.root)[MAP_REL].decode("utf-8")
        screens = (
            "| key | route | kind | section | parent | access | label | wave | story |\n"
            "|---|---|---|---|---|---|---|---|---|\n"
            "| screen:abs/operations/list | /operations | place | Operations |  | listOperations | Operations | 1 |  |\n"
        )
        self.assertEqual(out, self.expected_map(screens=screens))


class BreakdownTest(MapsTestCase):
    def test_exact_breakdown_in_slug_order(self):
        out = gen_maps.render(self.root)
        expected = BREAKDOWN_HEAD + BREAKDOWN_ROWS
        self.assertEqual(out[STREAM_REL + "/main/BREAKDOWN.md"], expected.encode("utf-8"))

    def test_story_flip_plus_regeneration_changes_breakdown(self):
        # BREAKDOWN columns carry wave, not status, so the flip that must show up is a wave flip.
        rel = STREAM_REL + "/main/BREAKDOWN.md"
        before = gen_maps.render(self.root)[rel]
        self.assertIn("| story:abs/operations/documents | 1 |", before.decode("utf-8"))
        self.write(
            STREAM_REL + "/main/stories/documents/story.md",
            STORY_DOCUMENTS.replace("wave: 1", "wave: 2"),
        )
        after = gen_maps.render(self.root)[rel]
        self.assertNotEqual(after, before)
        self.assertIn("| story:abs/operations/documents | 2 |", after.decode("utf-8"))

    def test_stream_without_stories_has_header_only(self):
        self.write(STREAM_REL + "/empty/stream.json", stream_json("abs", "operations", "empty"))
        out = gen_maps.render(self.root)
        expected = BREAKDOWN_HEAD.replace("stream:abs/operations/main", "stream:abs/operations/empty")
        self.assertEqual(out[STREAM_REL + "/empty/BREAKDOWN.md"], expected.encode("utf-8"))

    def test_stream_without_usable_stream_json_is_not_rendered(self):
        self.write(STREAM_REL + "/broken/stream.json", "{not json\n")
        self.write(STREAM_REL + "/broken/stories/a/story.md", STORY_DOCUMENTS)
        self.assertNotIn(STREAM_REL + "/broken/BREAKDOWN.md", gen_maps.render(self.root))

    def test_breakdown_block_sets_title_note_and_columns(self):
        profile = dict(UI_MIGRATION, name="team-flow", breakdown=RU_BREAKDOWN)
        self.write(
            ".agents/profiles/team-flow/profile.json",
            json.dumps(profile, indent=2, ensure_ascii=False) + "\n",
        )
        self.write(
            STREAM_REL + "/main/stream.json",
            stream_json("abs", "operations", "main").replace('"ui-migration"', '"team-flow"'),
        )
        out = gen_maps.render(self.root)
        expected = (
            "# Разбивка stream:abs/operations/main\n"
            "\n"
            "Сгенерировано tools/generate.py из stories/*/story.md. Не редактировать.\n"
            "\n"
            "| Стори | Волна | Экраны | Без экрана | Трекер | Зависит от |\n"
            "|---|---|---|---|---|---|\n"
            + BREAKDOWN_ROWS
        )
        self.assertEqual(out[STREAM_REL + "/main/BREAKDOWN.md"], expected.encode("utf-8"))

    def test_stream_with_unknown_profile_uses_default_breakdown(self):
        self.write(
            STREAM_REL + "/main/stream.json",
            stream_json("abs", "operations", "main").replace('"ui-migration"', '"missing"'),
        )
        out = gen_maps.render(self.root)
        self.assertEqual(
            out[STREAM_REL + "/main/BREAKDOWN.md"], (BREAKDOWN_HEAD + BREAKDOWN_ROWS).encode("utf-8")
        )

    def test_story_with_broken_frontmatter_is_skipped(self):
        self.write(STREAM_REL + "/main/stories/reports/story.md", "---\nkey: story:x\n")
        out = gen_maps.render(self.root)[STREAM_REL + "/main/BREAKDOWN.md"].decode("utf-8")
        self.assertIn("| story:abs/operations/documents |", out)
        self.assertNotIn("story:abs/operations/reports", out)


class RepeatTest(MapsTestCase):
    def test_render_twice_equal(self):
        first = gen_maps.render(self.root)
        self.write_out(first)
        self.assertEqual(gen_maps.render(self.root), first)

    def test_render_all_includes_map_outputs(self):
        rendered = generate.render_all(self.root)
        for rel, data in gen_maps.render(self.root).items():
            self.assertEqual(rendered.get(rel), data, rel)
        self.assertIn(MAP_REL, rendered)
        self.assertIn(STREAM_REL + "/main/BREAKDOWN.md", rendered)

    def run_script(self):
        return subprocess.run(
            [sys.executable, str(SCRIPT)],
            cwd=str(self.root),
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            universal_newlines=True,
        )

    def test_generate_script_second_run_changes_nothing(self):
        first = self.run_script()
        self.assertEqual(first.returncode, 0, first.stderr)
        self.assertIn(SCREENS_TABLE, (self.root / MAP_REL).read_text(encoding="utf-8"))
        self.assertTrue((self.root / STREAM_REL / "main/BREAKDOWN.md").is_file())
        second = self.run_script()
        self.assertEqual(second.returncode, 0, second.stderr)
        self.assertIn("generated: 0 files changed", second.stdout)

    def test_map_without_markers_is_left_byte_identical_by_generate(self):
        # The generator has no block to render; reporting the missing markers is check's business.
        hand = "# Map of domain operations\n\nHand text only, no markers.\n"
        self.write(MAP_REL, hand)
        result = self.run_script()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual((self.root / MAP_REL).read_bytes(), hand.encode("utf-8"))


if __name__ == "__main__":
    unittest.main()
