...
def recipe_repository(ws, entry):
    ...
    row = "| %s | %s | %s | %s | %s | %s |" % (
        entry["name"], entry["forge"], entry["default_branch"], ", ".join(entry["roles"]),
...
def add_domain(ws, domain):
    write(ws, "domains/%s/MAP.md" % domain, profile_template(ws, "MAP").replace("<domain>", domain))
...
def add_screen(ws, domain, slug, route, parent, wave, transitions):
    ...
    filled = (profile_template(ws, "screen")
              .replace("<domain>/<slug>", "%s/%s" % (domain, slug))
              .replace("/<route>", route)
              ...
              .replace("wave: 1\n", "wave: %s\n" % wave))
    write(ws, "domains/%s/map/%s.md" % (domain, slug), filled + rows)
...
def recipe_breakdown_row(ws, domain, stream, slug):
    ...
    if not path.exists():
        ...
        write(ws, rel, "# %s stream:%s/%s\n\n%s\n\n| %s |\n|---|---|---|---|---|---|\n"
              % (title, domain, stream, note, " | ".join(columns)))
...
class Recipes(unittest.TestCase):
    ...
    def test_repository(self):
        ...
        self.assertIn("| payments-worker | github | develop | backend, qa |  | Consumes",
    ...
    def test_repository_with_kit(self):
        ...
        with_kit = {"name": "abs-operations", "remote": "git@example.com:t/abs-operations.git", "forge": "gitlab",
                    "default_branch": "master", "roles": ["frontend"], "summary": "Operations micro frontend",
        ...
        for entry in (plain, with_kit):
            for ws in self.both:
                add_repository_source(ws, entry)
            recipe_repository(self.recipe, entry)
        self.assertIn("| abs-operations | gitlab | master | frontend | mfe | Operations micro frontend |\n",
                      (self.recipe / "REPOSITORIES.md").read_text(encoding="utf-8"))
        self.assertSameTrees()
    ...
    def test_map_element_takes_from_column_from_profile(self):
        for ws in self.both:
            ...
            add_screen(ws, "risks", "overview", "/risks", "", 1, [("Open alerts", "screen:risks/alerts")])
        recipe_map_element(self.recipe, "risks", "overview")
        self.assertIn("| %s | " % from_column(self.recipe), (self.recipe / "domains/risks/MAP.md").read_text(encoding="utf-8"))
        self.assertNotIn("| From | ", (self.recipe / "domains/risks/MAP.md").read_text(encoding="utf-8"))
        self.assertSameTrees()
...
class RussianRecipes(Recipes):
    ...
    def test_recipes_write_russian_profile_values(self):
        ...
        for ws in self.both:
            add_domain(ws, "operations")
            add_screen(ws, "operations", "reports", "/reports", "", 2, [("Назад", "screen:operations/users")])
            add_stream(ws, "operations", "migration")
            add_story(ws, "operations", "migration", "reports", 2, scope=["screen:operations/reports"])
        recipe_map_element(self.recipe, "operations", "reports")
        recipe_breakdown_row(self.recipe, "operations", "migration", "reports")
        recipe_story_index(self.recipe, "operations", "migration", "reports")
        map_text = (self.recipe / "domains/operations/MAP.md").read_text(encoding="utf-8")
        self.assertIn(
            "<!-- map:%s:begin -->\n| %s | %s |\n" % (heading.lower().replace(" ", "-"), from_value, " | ".join(columns)),
            map_text)
        breakdown_text = (self.recipe / "domains/operations/streams/migration/BREAKDOWN.md").read_text(
            encoding="utf-8")
