...
| Element-owned | one folder per domain, one file per map element | the merge request that changes that element | a person resolves it |
| Generated | `.claude/`, `.opencode/`, `.cursor/`, `CLAUDE.md`, the table in `REPOSITORIES.md`, `.agents/index.json` | `python3 tools/generate.py` or a skill recipe | take the default branch version, then regenerate |
| Shared hand-written | `AGENTS.md`, `README.md`, the text of `REPOSITORIES.md` outside the table, `.agents/rules/`, `.agents/roles/`, `.agents/skills/`, `.agents/agents/`, `.agents/templates/`, `.agents/kit.json`, `.agents/env.schema.json`, `tracker/tracker.json`, `docs/`, `repos.json`, `environments.json` | reviewed merge requests | stop and show both sides to the person |
