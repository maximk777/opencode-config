...
| Shared hand-written | `AGENTS.md`, `README.md`, the text of `REPOSITORIES.md` outside the table, `.agents/rules/`, `.agents/roles/`, `.agents/skills/`, `.agents/agents/`, `.agents/templates/`, `.agents/kit.json`, `.agents/env.schema.json`, `tracker/tracker.json`, `docs/`, `repos.json`, `environments.json`, `stream.json`, `epic.md`, the `Legacy trace` table and all other text of `MAP.md` outside the `map:` markers, `MOCKUPS.md` | reviewed merge requests | stop and show both sides to the person |
