---
name: repos-sync
description: Use to update clones of your selected repositories under repos/, or of the whole manifest with `all`, and see their branch, lag and uncommitted changes.
---
# Repository sync
...
3. Build the repository list. When the person asked for the option `all`, open `repos.json` and take every entry of `repositories`. Otherwise open `.local/repos.json` and read its `repositories` list; for every name in it, take the entry of `repos.json` with that `name`. Then, still in `.local/repos.json`, for every name in `add`, take the entry of `repos.json` with that `name`; when there is none, put the name in the list anyway and mark it "not in repos.json". Remove every entry whose `name` is in `skip`. When a `name` appears more than once, for example a name in `add` that `repositories` already includes, keep it once. Use each entry's `name`, `remote`, `default_branch` and `status`. An entry whose `status` is `planned` does not exist yet and may have no `remote`, so it is never cloned or fetched.
