...
// Idle artifacts for non-scoped agents: plain markdown, indexed vectors-only, no LLM extraction.
const ARTIFACT_USER_CHARS = 280;
const ARTIFACT_ASSISTANT_CHARS = 1200;
const ARTIFACT_MAX_MESSAGES = 16;
const ARTIFACT_MAX_CHARS = 10_000;

export function artifactPayload({ agent, title, messages }) {
  const text = (m) => (m?.parts ?? []).filter((p) => p?.type === "text").map((p) => p.text ?? "").join("\n").trim();
  const turns = [];
  for (const m of messages ?? []) {
    if (m.role !== "user" && m.role !== "assistant") continue;
    const body = text(m);
    if (body) turns.push({ role: m.role, body });
  }
  if (!turns.some((t) => t.role === "assistant")) return null;
  const out = [`# ${title || "Untitled session"}`, `- agent: ${agent}`, ""];
  for (const t of turns.slice(-ARTIFACT_MAX_MESSAGES)) {
    const cap = t.role === "user" ? ARTIFACT_USER_CHARS : ARTIFACT_ASSISTANT_CHARS;
    const block = [`## ${t.role}`, "", t.body.slice(0, cap), ""];
    if (out.join("\n").length + block.join("\n").length > ARTIFACT_MAX_CHARS) break;
    out.push(...block);
  }
  return { kind: "session-artifact", agent, text: out.join("\n") };
}

export const ARTIFACT_MIN_INTERVAL_MS = 900_000;

export function artifactDecision({ now, lastWriteAt, changed }) {
  if (!changed) return { write: false, reason: "unchanged" };
  if (lastWriteAt != null && now - lastWriteAt < ARTIFACT_MIN_INTERVAL_MS) return { write: false, reason: "interval" };
  return { write: true };
}

// Collision-safe change detection: a clash only skips one write.
export function fingerprint(text) {
  let h = 5381;
  for (let i = 0; i < String(text).length; i++) h = ((h * 33) ^ String(text).charCodeAt(i)) >>> 0;
